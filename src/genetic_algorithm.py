...
def genetics(n_generation = 6, population_size = 30, n_selected = 5, simulation_steps = 100,
    ...
    for generation in range(n_generation):
        ...
        [fitness_history[i].append(fitness[i]) for i in range(1)]
        ...
        if generation > 0 and generation == n_generation - 1:

            pd.DataFrame(fitness_history[0]).to_csv(main.RESULT_DIR + "room1.csv", header = None, index = 0)
            pd.DataFrame(fitness_history[1]).to_csv(main.RESULT_DIR + "room2.csv", header = None, index = 0)
            pd.DataFrame(fitness_history[2]).to_csv(main.RESULT_DIR + "room3.csv", header = None, index = 0)
            pd.DataFrame(np.array(stats)[:, 3]).to_csv(main.RESULT_DIR + "diversity.csv", header = None, index = 0)
